...
save "$clean/data_easi.dta", replace
...
save "$estimates/easi7.dta", replace
...
mat COEF = COEF2
outtable using "$estimates/EASI7_boot", mat(COEF) longtable nobox center f(%9.3f) replace

mat colnames COEF = food energy transport housing services durables
* Note that here, every other row is the standard errors
qui xml_tab COEF,  save($estimates/EASI_ouput_7_boot.xml) replace  title("Coefficients EASI: 7 Groups") sheet("Coefficients")
restore
...
qui xml_tab M_COEF,  save($estimates/EASI_ouput_7.xml) replace  title("Coefficients EASI: 7 Groups") sheet("Coefficients")
qui xml_tab M_COEF,  save($simulation/01-input/EASI_ouput_7.xls) replace  title("Coefficients EASI: 7 Groups") sheet("Coefficients")
