********************************************************************************
* 1. EASI Demand System with 7 Groups (baseline analysis) and urban dummy
* Author: Claire Paoli
* Date: March 2021
...
replace out_layers=1 if  bsx`v'<r(c_1) | bsx`v'>r(c_2)
}
count if out_layers==1
keep if out_layers==0
drop out_layers
...
g equiv_income = income_wk_tot/oecdsc
...
// ------------+------------------------------------
//    LCF_2006 |    41035714           0       4,220
//    LCF_2007 |    40735506   7.452e-09       3,807
//    LCF_2008 |    41483760           0       3,642
//    LCF_2009 |    40815570           0       3,515
//    LCF_2010 |    40849546           0       3,219
//    LCF_2011 |    40531795   7.452e-09       3,516
//    LCF_2012 |    41190106           0       3,513
//    LCF_2013 |    44589192           0       3,361
//    LCF_2014 |    43265988           0       3,292
//    LCF_2015 |    44134313           0       3,202
//    LCF_2016 |    46530361           0       3,377
//    LCF_2017 |    46213249           0       3,556
//    LCF_2018 |    45817404           0       3,535
// ------------+------------------------------------
//       Total |    42805213   2226091.2      45,755
...
su xtot savings income_paye_tax hours_tot inc_wk_salary_tot income_wk_tot wage_notax hours_tot

replace wage_notax = 0 if wage_notax==.
g inc_wk_nonsalary_tot = income_wk_tot - inc_wk_salary_tot
replace deductions= 0 if deductions==.
replace income_wk_disposable = income_wk_tot - deductions
...
tab x_decile if year==2017, su(agg_income_share)
tab x_decile if year==2017, su(agg_x_share)
tab x_decile if year==2017, su(equiv_disp_income)
tab x_decile if year==2017, su(equiv_consumption)
...
*Urban rural only for 2015-2018
...
eststo clear
eststo sum1: quietly estpost summarize ///
    unemployed employed student retiree inc_wk_salary_tot inc_wk_nonsalary_tot xtot ///
	income_wk_tot income_wk_disposable wage_notax exp_share wk_hours_worked_hrp_tot wk_hours_worked_spouse_tot income_paye_tax ///
	...
	North_East North_West Yorkshire East_Midlands West_Midlands Eastern London South_East South_West Wales Scotland Northern_Ireland [aw=weight_adjusted]
esttab sum1 using "$estimates/sum1.tex", cells("mean(fmt(2)) sd(fmt(2)) min max") label nodepvar title(Summary Statistics) replace

eststo sum2: quietly estpost tab ///
    het_household
esttab sum2 using "$estimates/sum2.tex", title(Household Types) label nodepvar replace
...
esttab sum3 using "$estimates/sum3.tex", cells("mean(fmt(2)) sd(fmt(2)) min max") label nodepvar title(Budget Shares and Prices) replace
...
* Controls for car ownership, region, month, year, fuel type, urban/rural
* gen d_cars = (Cars==1)
gen d_rent = (Rented==1)
gen d_flat = (Flat==1)

* Note: this is now a smaller sample. 
gen d_urban = urban
...
global var "d_* reg_* month_* i.year"
...
save "$clean/data_easi_urban.dta", replace
...
* Prices are normalized so that the price vector in Jan 2006 = (1,..,1), such that the log is p = 0
...
*esttab using "$estimates/EASI_output_7_urban.csv", cells(b(star fmt(3)) se(par fmt(2))) legend label varlabels(_cons Constant) stats(r2 df_r bic, fmt(3 0 1) label(R-sqr)) replace

*estimates store EASI
*estwrite  using "$estimates/EASI_output_7_urban", replace
...
simulate, reps(10) seed(1): myboot
save "$estimates/easi7_urban.dta", replace
...
mat temp1 = COEF[1..102,1...]
mat temp2 = COEF[103..204,1...]
mat temp3 = COEF[205..306,1...]
mat temp4 = COEF[307..408,1...]
mat temp5 = COEF[409..510,1...]
mat temp6 = COEF[511..612,1...]

mat COEF2 = temp1,temp2,temp3,temp4,temp5,temp6
mat COEF = COEF2
outtable using "$estimates/EASI7_boot_urban", mat(COEF) longtable nobox center f(%9.3f) replace

mat colnames COEF = food energy transport housing services durables
* Note that here, every other row is the standard errors
qui xml_tab COEF,  save($estimates/EASI_ouput_7_boot_urban.xml) replace  title("Coefficients EASI: 7 Groups") sheet("Coefficients")
restore
...
graph export "$figures/bs1.png", replace

predict bs2, equation(s2)
quietly twoway (qfitci bsxheating lxtot) (qfitci bs2 lxtot), ytitle("Budget Share") xtitle("Log Tot Expenditure")  legend(label(2 "Quadratic Fit - Data") label(3 "Quadratic Fit - 3SLS")) title("Domestic Energy")
graph export "$figures/bs2.png", replace
...
graph export "$figures/bs3.png", replace
...
graph export "$figures/bs4.png", replace
...
graph export "$figures/bs5.png", replace
...
graph export "$figures/bs6.png", replace
...
graph export "$figures/engel_`good'.png", replace
}
...
qui xml_tab M_COEF,  save($estimates/EASI_ouput_7_urban.xml) replace  title("Coefficients EASI: 7 Groups") sheet("Coefficients")
qui xml_tab M_COEF,  save($simulation/01-input/EASI_ouput_7_urban.xls) replace  title("Coefficients EASI: 7 Groups") sheet("Coefficients")
...
save "$clean/data-labor-supply.dta", replace
...
*************************************************
use "$estimates/easi7.dta", clear
 
eststo clear
eststo income: quietly estpost summarize ///
   
esttab sum1 using "$estimates/income_elas.tex", cells("mean(fmt(3)) sd(fmt(3))") label nodepvar title(Income Elasticities) replace

eststo income: quietly estpost summarize ///
   
esttab sum1 using "$estimates/H_elast.tex", cells("mean(fmt(3)) sd(fmt(3))") label nodepvar title(Hicksian Elasticities) replace

eststo income: quietly estpost summarize ///
   
esttab sum1 using "$estimates/M_elas.tex", cells("mean(fmt(3)) sd(fmt(3))") label nodepvar title(Marshallian Elasticities) replace
