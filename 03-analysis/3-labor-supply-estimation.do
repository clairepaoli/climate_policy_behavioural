...
** How many hhs with more than 2 adults?
count if n_adults > 2
* 9,430
...
* single males;
count if n_adults == 1 & sexhrp == 1
* 4,491

* single female;
count if n_adults == 1 & sexhrp == 2
* 8,151
...
su income_spouse_labor_gross income_spouse_self wk_hours_worked_spouse_tot income_hrp_labor_gross income_hrp_self wk_hours_worked_hrp_tot

// g spouse_hours = (income_spouse_labor_gross + income_spouse_self) / wk_hours_worked_spouse_tot
// g hrp_hours = (income_hrp_labor_gross + income_hrp_self) / wk_hours_worked_hrp_tot

// gen income_taxable_hrp = inc_wk_salary_tot + inc_wk_self + income_wk_pension
// gen income_taxable_spouse = inc_wk_salary_tot + inc_wk_self + income_wk_pension
...
esttab mo1 mo2 mo3 mo4 mo5 using "$estimates/2025.03.29_labor-supply_wself.tex", cells(b(star fmt(3)) se(par fmt(2))) legend label varlabels(_cons Constant) stats(N r2, label(N R-Sqr)) title(Labour Supply: UK Households, Total) replace
...
esttab mo7 mo8 mo9 mo10 m011 using "$estimates/2025.03.29_labor-supply-couples.tex", cells(b(star fmt(3)) se(par fmt(2))) legend label varlabels(_cons Constant) stats(N r2, label(N R-Sqr)) title(Labour Supply: UK Households, Couples) replace
...
esttab mo20 mo21 mo22 mo23 mo24 using "$estimates/2025.03.29_labor-supply-singles.tex", cells(b(star fmt(3)) se(par fmt(2))) legend label varlabels(_cons Constant) stats(N r2, label(N R-Sqr)) title(Labour Supply: UK Households, Singles) replace
restore
...
estwrite  using "$estimates/2025.03.29_labor-supply", replace
