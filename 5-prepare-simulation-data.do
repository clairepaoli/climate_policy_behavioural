...
* Merge with 2017 emissions data
sort LCF_round year hh_id
merge LCF_round year hh_id using "$clean/data_emissions.dta"
keep if _merge == 3
drop if year==.
drop _merge
...
estread using "$estimates/EASI_output_7"
...
gen d_working_m = (group==2 | group==4)
gen d_working_f = (group==1 | group==3)
...
gen wage_dxy_1 = wage_tax/dxy
* use wage_dx_2 in estimation!
...
* Gender and occupation specific only (nationa net wage)
...
estread using "$estimates/labor-supply"
estimates restore LABOR

mat COEF=e(b)
mat COEF2 = COEF'
mat colnames COEF2 = IV 
qui xml_tab COEF2,  save($estimates/labour_fn.xls) replace  title("Labour Supply")
...
// ********************************************************************************
// * Randomly sample 1000 households;
...
hh_id LCF_round weight_adjusted_final
hh_size exp_poverty urban n_children n_adults region

inc_wk_salary_tot inc_wk_self inc_wk_invest income_wk_ss income_wk_tot income_wk_disposable income_taxable savings income_annual
income_transfers ni_contribution income_paye_tax income_paye_tax_net deductions income_hrp_labor_gross income_spouse_labor_gross dxy
exp_share oecdsc equiv_income equiv_disp_income equiv_consumption 

wk_hours_worked_hrp wk_hours_worked_spouse hours_tot

wage_dxy_2 wage_tax2

mar_tax aver_tax 

het_household

heck_agg sexhrp london d_children group single

xtot xfood xelec xheating xtransport xhousing xserv xdur xother2	
...
food_int heating_int elec_int transport_int housing_int serv_int dur_int other2_int
food_emi heating_emi elec_emi transport_emi housing_emi serv_emi dur_emi other_emi tot_hh_footprint

income_wk_pension
food_int_c heating_int_c transport_int_c housing_int_c serv_int_c dur_int_c other_int_c
d_urban
d_eletric_heating
d_gas_heating
dummy_month*
...
egen tot_exp_food = total(xfood*weight_adjusted_final)
egen tot_exp_housing = total(xhousing*weight_adjusted_final)
egen tot_exp_heating = total(xheating*weight_adjusted_final)
egen tot_exp_transport = total(xtransport*weight_adjusted_final)
egen tot_exp_services = total(xserv*weight_adjusted_final)
egen tot_exp_durables = total(xdur*weight_adjusted_final)
egen tot_exp_others = total(xother2*weight_adjusted_final)
...
savings
income_annual
income_hrp_labor_gross
wk_hours_worked_hrp
wk_hours_worked_spouse
income_spouse_labor_gross
mar_tax
hours_tot
aver_tax
wage_tax2
...
export excel using "$simulation/01-input/simulation_data.xls", firstrow(variables) nolabel replace	
restore
